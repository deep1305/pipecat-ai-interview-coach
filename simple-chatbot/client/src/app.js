...
class VoiceChatClient {
  ...
  setupDOM() {
    ...
    if (this.jdTextarea) {
      this.jdTextarea.addEventListener('input', () => {
        const len = this.jdTextarea.value.trim().length;
        this.jdCharCount.textContent = `${len} characters`;
      });
    }
  ...
  async connect() {
    ...
    try {
      ...
      this.addEvent('config-saved', `Configuration saved: ${result.message}`);
    } catch (error) {
      this.addEvent('config-error', `Failed to save config: ${error.message}`);
      // Continue with connection anyway - backend will use defaults
      ...
      if (!participant?.local) {
        if (track.kind === 'audio') {
          ...
          audio.play().catch((err) => {
            this.addEvent('audio-error', `Autoplay blocked: ${err.message} — click anywhere on page`);
